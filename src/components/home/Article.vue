<template>
	<div class="ArticleSection">
		...
		<div class="article" v-else>
			...
			<ul>
				<li>• 分类:{{ dataTab }}</li>
				<li>• {{ data.visit_count }}次访问</li>
				<li>• 发布于:{{ data.create_at | formatDate }}</li>
				<li>
					• 作者：
					<router-link :to="{ name:'user_info', params: { name:data.author.loginname } }">{{ data.author.loginname }}</router-link>
				</li>
			</ul>
			<div v-html="data.content" id="content"></div>
		</div>
		<div id="reply">
			<div v-for='(reply, index) in data.replies' class="replySec">
				<div class="replyUp">
...
export default {
  	...
  	computed: {
  		dataTab() {
  			var type = this.data.tab.toString();
